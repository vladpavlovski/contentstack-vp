      ...
      <NavigationMenuList>
        {navigation_menu.map((menuItem) => {
          ...
          return hasDropdown ? (
              ...
              <NavigationMenuContent>
                <ul className='grid gap-3 p-4 md:w-[400px] lg:w-[500px] lg:grid-cols-[.75fr_1fr]'>
                  {menuItem.navigation_submenu.map((subMenu) => {
                    return (
                      <ListItem key={subMenu._metadata.uid} href={subMenu.page_reference[0].url} title={subMenu.label}>
            ...
            <Link key={menuItem._metadata.uid} href={menuItem.page_reference[0].url} legacyBehavior passHref>
              <NavigationMenuLink className={navigationMenuTriggerStyle()}>{menuItem.label}</NavigationMenuLink>
            </Link>
          )
        })}
      </NavigationMenuList>
