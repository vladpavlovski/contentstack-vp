...
export const isBasicConfigValid = () => {
  return !!CONTENTSTACK_API_KEY && !!CONTENTSTACK_DELIVERY_TOKEN && !!CONTENTSTACK_ENVIRONMENT
}
...
const setRegion = (): Region => {
  ...
  if (!!CONTENTSTACK_REGION && CONTENTSTACK_REGION !== 'us') {
    region = CONTENTSTACK_REGION.toLocaleUpperCase().replace('-', '_') as keyof typeof Region
  }
